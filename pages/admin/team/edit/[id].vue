                ...
                <div class="col-12">
                    <div class="">
                        <div class="">
                            ...
                            <div class="form-group mt-10">
                                <label for="" class="label">ตำแหน่ง : </label>
                                <div>
                                    <input
                                        type="text"
                                        class="form-control form-control-plaintext"
                            ...
                            <div class="form-group mt-10">
                                <label for="" class="label">งาน : </label>
                                <div>
                                    <input
                                        type="text"
                                        class="form-control form-control-plaintext"
                                ...
                                <div>
                                    <client-only>
                                        <v-select
                                            ...
                                            :options="
                                                selectOptions.department_teams
                                            "
                                            id="slt-department-team"
                                            v-model="item.department_team_id"
...
const fetchDepartments = async () => {
    let data = await $fetch(`${apiBase}/department-team`, {
        params: {
            is_publish: 1,
            perPage: 100,
        },
    ...
    selectOptions.value.department_teams = data.data.map((e) => {
        return { name: e.name, id: e.id };
    });
};

const { data: res } = await useFetch(`${apiBase}/team/${route.params.id}`, {
    server: true,
});

item.value = { ...res.value.data };
item.value.team_file_old = res.value.data.team_file;
item.value.team_file = null;

item.value.department_team_id = {
    name: res.value.data.department_team.name,
    id: res.value.data.department_team_id,
};
...
const onSubmit = async () => {
    let type_object = {
        ...
        method: "put",
        url: apiBase + "/team/" + item.value.id,
    };

    let data = {
        ...item.value,
        team_file: file.value.files != null ? file.value.files[0] : null,
        department_team_id: item.value.department_team_id?.id ?? undefined,
